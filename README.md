...
- **书签 / 页码跳转**：`Ctrl+E` 添加书签（记录章节与位置），`Ctrl+Shift+E` 书签列表（跳转/删除/清空），`Ctrl+G` 跳转到指定页
- **排版**：字号 / 行距 / **8 套皮肤**（代码伪装、Word、纸白、护眼绿、暗色、终端黑底绿字、浏览器网页、微信灰底）
- **自动隐藏**：设置里可选「失去焦点自动隐藏」「鼠标移出后自动隐藏（约 0.4 秒延迟）」，默认关闭
- **点击穿透悬浮窗**：托盘/菜单开关，开启后鼠标可点穿阅读窗口到下层应用，配合自动滚动即可"边工作边瞄"
...
- 点击穿透开启后窗口不响应鼠标（这就是目的），请用托盘菜单或全局老板键关闭。
- 失焦/移出自动隐藏默认关闭；开启后若被误藏，用托盘"显示/隐藏"或老板键恢复。
